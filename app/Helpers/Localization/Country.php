...
class Country
{
	...
	// Maxmind Support URL
	private static $maxmindSupportUrl = 'https://support.bedigit.com/help-center/articles/14/enable-the-geo-location';
	...
	public function validateTheCountry()
	{
		...
		if (
			...
			|| Str::endsWith($this->request->url(), '.css')
		) {
			return;
		}
	 ...
	 * @return \Illuminate\Support\Collection
	 */
	public static function setCountryToCookie($countryCode): \Illuminate\Support\Collection
	{
		if (trim($countryCode) == '') {
			return collect();
		}
		
		Cookie::set('ipCountryCode', $countryCode, self::$cookieExpiration);
	 ...
	 * @return \Illuminate\Support\Collection
	 */
	public static function getCountryFromCookie(): \Illuminate\Support\Collection
	{
		if (Cookie::has('ipCountryCode')) {
	 ...
	 * @return \Illuminate\Support\Collection
	 */
	public static function getCountries(): \Illuminate\Support\Collection
	{
		...
		// Sort
		$countries = Arr::mbSortBy($countries, 'name', app()->getLocale());
